...
% 增加网格密度提高平滑度
kza_fine = linspace(min(kza_coarse), max(kza_coarse), 100);
kzb_fine = linspace(min(kzb_coarse), max(kzb_coarse), 100);
...
% 显示优化结果信息
fprintf('\n=== 可视化结果信息 ===\n');
fprintf('S_q2_1 最优参数: k_{zn,a} = %.0f, P_{avg} = %.6g W\n', k_zn_a_max, P_avg_vec(idx_max));
fprintf('S_q2_2 最优参数: k_{zn,a} = %.0f, k_{zn,b} = %.2f, P_{avg} = %.6g W\n', ...
